...
Prometheus is a data monitoring solution. CONNMIX has a built-in Prometheus indicator interface `/metrics`, which allows us to keep track of the status of the system at any time, quickly locate problems and troubleshoot them.

> This function is only available for the professional version

## `center` node monitoring

You can view all indicators on the default port `6787` of `apiServer` on this node:
...
## `engine` node monitoring

You can view all indicators on the default port `6789` of `apiServer` on this node:
